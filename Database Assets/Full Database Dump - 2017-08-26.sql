...
CREATE TABLE `study_participants` (
  ...
  CONSTRAINT `FK_study_participants_participant` FOREIGN KEY (`participant_id`) REFERENCES `participants` (`participant_id`) ON UPDATE CASCADE,
  CONSTRAINT `FK_study_participants_participant_status` FOREIGN KEY (`participant_status`) REFERENCES `study_participant_status` (`id`) ON UPDATE CASCADE,
  CONSTRAINT `FK_study_participants_user_group` FOREIGN KEY (`study_user_group`) REFERENCES `study_user_groups` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
