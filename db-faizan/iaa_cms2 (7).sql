...
--
-- Host: 127.0.0.1
-- Generation Time: Aug 29, 2017 at 06:10 AM
...
(3, 2, 0, 1, 1, 1, 0, '0000-00-00', '0000-00-00', 1, 0, 3, 3, 3, 3, '3', '', '', '');
...
CREATE  VIEW `v_active_users`  AS  select `users`.`user_id` AS `user_id`,concat(`users`.`first_name`,' ',`users`.`last_name`) AS `uname`,`users`.`username` AS `username` from `users` where (`users`.`active` = 1) order by concat(`users`.`first_name`,' ',`users`.`last_name`) ;
...
ALTER TABLE `screener_answers`
  ADD KEY `FK_screener_answers_study` (`study_id`),
...
--
ALTER TABLE `participants_autosave`
  MODIFY `participant_autosave_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
